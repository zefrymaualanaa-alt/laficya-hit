...
bot.command('xxxxx', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) {
        return ctx.reply("❌ *Akses Ditolak! Perintah ini khusus Owner LAFICYA Store.*", { parse_mode: 'Markdown' });
    }
...
async function processNextHit() {
    ...
    try {
        ...
        if (allResults.length > 0) {
            ...
            await ctx.reply(
                `<b>[ LAFICYA ENTERPRISE - RESULT SUMMARY ]</b>\n` +
                `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
...
bot.command('catat', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) return ctx.reply("❌ *Akses Khusus Owner LAFICYA Store!*", { parse_mode: 'Markdown' });
...
bot.command('ownerbulk', async (ctx) => {
    if (ctx.from.id !== OWNER_ID) return ctx.reply("❌ *Perintah ini eksklusif hanya untuk Owner LAFICYA Store!*", { parse_mode: 'Markdown' });
