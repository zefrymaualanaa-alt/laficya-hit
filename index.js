...
// --- FITUR AUTO CHANGE CC (CAPCUT) ---
async function runCapcutAutomation(gmail, gpass) {
    let browser;
    try {
        // Gunakan globalBrowser jika ada, atau buat instance baru
        browser = await chromium.launch({ headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox'] });
        const context = await browser.newContext();
        const page = await context.newPage();
        
        // Buka tab baru untuk Temp Mail (Sesuai video lu: generator.email)
        const mailPage = await context.newPage();
        await mailPage.goto('https://generator.email/', { waitUntil: 'networkidle' });
        
        // Scraping email baru dari temp mail
        const newEmail = await mailPage.$eval('#email_ch_text', el => el.textContent.trim());

        // Mulai eksekusi CapCut di tab utama
        await page.goto('https://www.capcut.com/account-center/login', { waitUntil: 'networkidle' });
        
        // 1. Klik Login via Google
        // Catatan: Google kadang minta verifikasi kalau headless, pastikan IP VPS lu bersih
        await page.click('text="Continue with Google"');
        
        // Tunggu popup/navigasi Google Login
        ...
        // 2. Masuk ke halaman Sign-in and Management (sesuai video)
        await page.goto('https://www.capcut.com/account-center/bind_email=0', { waitUntil: 'networkidle' });
        
        // Klik tombol Change / Link Email
        await page.click('text="Link"'); // Sesuaikan selector 'Link' atau 'Change'
        await page.fill('input[placeholder="Enter email"]', newEmail);
        await page.click('button:has-text("Send code")');

        // 3. Ambil OTP dari Temp Mail
        await mailPage.bringToFront();
        await mailPage.waitForTimeout(10000); // Tunggu email masuk
        await mailPage.reload();
        // Ekstrak kode angka (misal subjeknya: "613207 CapCut...")
        const otpText = await mailPage.$eval('.message_subject', el => el.textContent.trim()); 
        const otpCode = otpText.match(/\d{6}/)[0]; // Ambil 6 digit OTP

        // 4. Masukkan OTP ke CapCut
        await page.bringToFront();
        await page.fill('input[placeholder="Enter code"]', otpCode);
        await page.click('button:has-text("Confirm")');
        await page.waitForTimeout(3000);

        // 5. Logout & Reset Password
        await page.click('text="Sign out"');
        await page.waitForLoadState('networkidle');
        
        await page.goto('https://www.capcut.com/id-id/forget-password');
        await page.fill('input[placeholder="Enter email"]', newEmail);
        await page.click('button:has-text("Confirm")');

        // 6. Ambil link Reset Password dari Temp Mail
        await mailPage.bringToFront();
        await mailPage.waitForTimeout(10000); // Tunggu email reset
        await mailPage.reload();
        const resetLink = await mailPage.$eval('a:has-text("Reset Password")', el => el.href);

        // 7. Buka link reset & set password default
        await page.goto(resetLink);
        await page.fill('input[placeholder="New password"]', 'masuk123'); // Sesuai request lu
        await page.fill('input[placeholder="Confirm password"]', 'masuk123');
        await page.click('button:has-text("Confirm")');
        
        await browser.close();
        
        return { success: true, newEmail: newEmail, password: 'masuk123' };
    } catch (error) {
...
bot.command('changecc', async (ctx) => {
    // Verifikasi Akses (Hanya Owner & VIP)
    ...
    if (args.length < 2 || !args[1].includes(':')) {
        return ctx.reply('⚠️ *Format salah!*\nGunakan format: `/changecc emailgmail@gmail.com:passwordgmail`', { parse_mode: 'Markdown' });
    }

    const [gmail, gpass] = args[1].split(':');

    const statusMsg = await ctx.reply(`⏳ <b>[CAPCUT SYSTEM]</b>\nSedang memproses pergantian email untuk:\n<code>${gmail}</code>...\n<i>Proses ini memakan waktu sekitar 30-60 detik.</i>`, { parse_mode: 'HTML' });

    // Eksekusi fungsi automasi Playwright
    const result = await runCapcutAutomation(gmail, gpass);

    if (result.success) {
        const layoutSuccess = 
            `✅ <b>CAPCUT EMAIL CHANGED SUCCESSFULLY</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            `<blockquote>` +
            `📧 <b>Email Asli:</b> <code>${gmail}</code>\n` +
            `📨 <b>Email Baru:</b> <code>${result.newEmail}</code>\n` +
            `🔑 <b>Password :</b> <code>${result.password}</code>\n` +
            `</blockquote>\n\n` +
            `<i>Silakan login ke aplikasi CapCut menggunakan Email Baru dan Password di atas.</i>`;
            
        await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, layoutSuccess, { parse_mode: 'HTML' });
    } else {
        await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, `❌ <b>GAGAL MEMPROSES CAPCUT:</b>\n<code>${result.message}</code>\n\n<i>Kemungkinan karena Google mendeteksi browser otomatis atau perubahan antarmuka website.</i>`, { parse_mode: 'HTML' });
    }
});
...
async function processNextHit() {
 ...
 await page.goto('https://kxntu.com/login', { waitUntil: 'domcontentloaded' });

console.log('URL:', page.url());
console.log('TITLE:', await page.title());
console.log('USERNAME:', await page.locator('#username').count());
    ...
    try {
        ...
        if (isTxt && userCurrentMode === 'LINK_TO_COOKIES') {
            ...
            if (linksToProcess.length === 0) {
                ...
                return ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, undefined, "❌ Tidak ditemukan link await page.goto('https://kxntu.com/login', { waitUntil: 'domcontentloaded' });
...
await page.fill('#username', account.username); Netflix yang valid di dalam file.");
            }
